export const ADD_CART_ITEM = 'ADD_CART_ITEM';
export const ADD_CART_ITEM_FAILED = 'ADD_CART_ITEM_FAILED';
export const REMOVE_CART_ITEM = 'REMOVE_CART_ITEM';
export const REMOVE_CART_ITEM_FAILED = 'REMOVE_CART_ITEM_FAILED';
export const GET_INITIAL_STATE = 'GET_INITIAL_STATE';
export const HIDRATE_INITIAL_STATE = 'HIDRATE_INITIAL_STATE';

// item details
export const GET_ITEM = 'GET_ITEM';
export const GET_ITEM_FAILED = 'GET_ITEM_FAILED';

// contact form
export const SEND_CONTACT_MSG_FAILED = 'SEND_CONTACT_MSG_FAILED';
export const SEND_CONTACT_MSG = 'SEND_CONTACT_MSG';

// admin
export const ADMIN_LOGIN = 'ADMIN_LOGIN';
export const ADMIN_LOGIN_FAILED = 'ADMIN_LOGIN_FAILED';
export const ADMIN_GET_IMAGES = 'ADMIN_GET_IMAGES';
export const ADMIN_GET_IMAGES_FAILED = 'ADMIN_GET_IMAGES_FAILED';
export const ADMIN_UPDATE_CATEGORIES = 'ADMIN_UPDATE_CATEGORIES';
export const ADMIN_UPDATE_CATEGORIES_FAILED = 'ADMIN_UPDATE_CATEGORIES_FAILED';
export const ADMIN_GET_PUBLISHED_PRODUCTS = 'ADMIN_GET_PUBLISHED_PRODUCTS';
export const ADMIN_GET_PUBLISHED_PRODUCTS_FAILED =
  'ADMIN_GET_PUBLISHED_PRODUCTS_FAILED';
export const ADMIN_GET_CATEGORIES = 'ADMIN_GET_CATEGORIES';
export const ADMIN_GET_CATEGORIES_FAILED = 'ADMIN_GET_CATEGORIES_FAILED';
export const ADMIN_ADD_PRODUCT = 'ADMIN_ADD_PRODUCT';
export const ADMIN_ADD_PRODUCT_FAILED = 'ADMIN_ADD_PRODUCT_FAILED';
